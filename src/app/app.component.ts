...
export class AppComponent {
  title = 'globe-demo';
  private countryData: {} | undefined;
  public countryDetails: string | undefined;
  constructor(private dataSer: DataService) {

    this.dataSer.getAssetsData().subscribe(x => {
      this.countryData = x;
      this.loadGlobe();
    },error => {
      this.countryData = {};
  ...
  private loadGlobe() {
    ...
    d3.json('assets/ne_110m_admin_0_countries.json', (err, d) => {
      map.append('g')
        ...
        .attr('d', path)
        .attr('fill', (d: any) => getScoreColour(this.getCountryScore(d.properties.ISO_A2, d.properties.NAME)))
        .style('stroke', 'black')
  ...
  private getCountryScore(countryCode: string, countryName: string): number | undefined {
    const country = this.countryData[this.getCountryCode(countryCode, countryName)];
    return country?.entitled ? country.score : undefined;
  }
  ...
  private getCountryCode(countryCode: string, countryName: string) : string {
    let code: string;
    switch (countryName) {
      case 'France':
      case 'Norway':
